...
namespace TibiaDataApi.Services.Tests
{
    public sealed class AssetStreamServiceTests
    {
        ...
        private sealed class StubAssetsDataBaseService(AssetStreamDescriptor? descriptor) : IAssetsDataBaseService
        {
            ...
            public Task<AssetMetadataResponse?> GetAssetMetaDataAsync(int assetId, CancellationToken cancellationToken = default)
            {
                return Task.FromResult<AssetMetadataResponse?>(null);
            }

            public Task<IReadOnlyList<AssetMetadataResponse>> SearchAssetMetadataByFileNameAsync(string fileName, CancellationToken cancellationToken = default)
            {
                return Task.FromResult<IReadOnlyList<AssetMetadataResponse>>([]);
            }
        }
    }
}
