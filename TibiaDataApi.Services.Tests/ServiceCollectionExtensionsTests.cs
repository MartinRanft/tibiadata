...
namespace TibiaDataApi.Services.Tests
{
    ...
    {
        [Fact]
        public void AddTibiaDataApiServices_RegistersCatalogScrapersAndWheelImportScraper()
        {
            ...
            IWheelDataImportService wheelImportService = provider.GetRequiredService<IWheelDataImportService>();
            IWheelPlannerLayoutSource wheelPlannerLayoutSource = provider.GetRequiredService<IWheelPlannerLayoutSource>();

            List<string> expectedCategorySlugs = TibiaWikiCategoryCatalog.All
                                                                         .Select(entry => entry.Slug)
                                                                         .Append("wheel-data-import")
            ...
            Assert.NotNull(wheelImportService);
            Assert.NotNull(wheelPlannerLayoutSource);
