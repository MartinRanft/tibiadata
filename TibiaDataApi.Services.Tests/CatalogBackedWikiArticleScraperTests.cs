...
namespace TibiaDataApi.Services.Tests
{
    public sealed class CatalogBackedWikiArticleScraperTests
    {
        ...
        [Fact]
        public void BuildArticle_HuntingPlaceInfobox_DoesNotShiftEmptyFieldsIntoNextParameter()
        {
            const string rawWikiText = """
                                       {{Infobox Hunt
                                       | name            = Bloodfire Gorge
                                       | image           = Raubritter Skirmisher
                                       | city            = Roshamuul
                                       | location        =
                                       | implemented     = 15.20.7a5cc9
                                       | vocation        = All
                                       | lvlknights      =
                                       | lvlpaladins     =
                                       | lvlmages        =
            ...
            const string renderedHtml = """
                                        <div class="mw-parser-output">
                                          <p>Underground, below the Stag Bastion.</p>
                                        </div>
                                        """;

            TestCatalogBackedWikiArticleScraper scraper = new(WikiContentType.HuntingPlace, "hunting-places");

            WikiArticle article = scraper.Parse("Bloodfire Gorge", rawWikiText, renderedHtml);
            Dictionary<string, string> infobox = JsonSerializer.Deserialize<Dictionary<string, string>>(article.InfoboxJson!)!;

            Assert.Equal("Roshamuul", infobox["city"]);
            Assert.Equal("15.20.7a5cc9", infobox["implemented"]);
            Assert.Equal("All", infobox["vocation"]);
            Assert.False(infobox.ContainsKey("location"));
            Assert.False(infobox.ContainsKey("lvlknights"));
            Assert.False(infobox.ContainsKey("lvlpaladins"));
            Assert.Null(article.AdditionalAttributesJson);
        }
