...
{
    public sealed class CreatureImageAssetService(
        ITibiaWikiHttpService tibiaWikiHttpService,
        AssetStorageOptions assetStorageOptions,
        IHostEnvironment hostEnvironment,
        ILogger<CreatureImageAssetService> logger) : ICreatureImageAssetService
    {
        private readonly AssetStorageOptions _assetStorageOptions = assetStorageOptions;
        private readonly IHostEnvironment _hostEnvironment = hostEnvironment;
        private readonly ILogger<CreatureImageAssetService> _logger = logger;
        ...
        {
            if(!_assetStorageOptions.DownloadCreatureImages || creature.Id <= 0)
            {
                return;
            }

            WikiImageInfo? wikiImage = await ResolvePrimaryImageAsync(wikiPageTitle, creature.Name, cancellationToken);
            CreatureAsset? existingRelation = await db.CreatureAssets
            ...
            string extension = ResolveExtension(wikiImage);
            string fileName = $"primary{extension}";
            string storageKey = $"{_assetStorageOptions.CreatureImageDirectory.Trim('/').Trim()}/{creature.Id}/{fileName}";
            ...
            else
            {
                ...
                asset.MimeType = wikiImage.MimeType;
            ...
            _logger.LogDebug(
                "Synchronized primary image for creature {CreatureName} to {StorageKey}.",
                creature.Name,
                storageKey);
        }
        ...
        private async Task RemovePrimaryImageAsync(
            TibiaDbContext db,
            CreatureAsset? existingRelation,
            CancellationToken cancellationToken)
        {
            ...
            db.CreatureAssets.Remove(existingRelation);

            bool hasOtherReferences = await db.CreatureAssets
            ...
            CancellationToken cancellationToken)
        {
            ...
            string selectedImageTitle = SelectPrimaryImageTitle(imageTitles, wikiPageTitle, creatureName);
            ...
            string creatureName)
        {
            ...
            string normalizedCreatureName = NormalizeTitle(creatureName);
            ...
            string? exactMatch = imageTitles.FirstOrDefault(title =>
            {
                ...
                return string.Equals(normalizedFileBaseName, normalizedPageTitle, StringComparison.OrdinalIgnoreCase) ||
                       string.Equals(normalizedFileBaseName, normalizedCreatureName, StringComparison.OrdinalIgnoreCase);
            });

            return exactMatch ?? imageTitles[0];
        }

        private static string ResolveExtension(WikiImageInfo wikiImage)
        {
            string extension = Path.GetExtension(wikiImage.FileTitle);
            if(!string.IsNullOrWhiteSpace(extension))
            {
                return extension.ToLowerInvariant();
            }

            return wikiImage.MimeType?.ToLowerInvariant() switch
            {
                "image/png" => ".png",
                "image/gif" => ".gif",
                "image/webp" => ".webp",
