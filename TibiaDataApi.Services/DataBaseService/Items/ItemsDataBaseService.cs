...
namespace TibiaDataApi.Services.DataBaseService.Items
{
        ...
        CachingOptions cachingOptions) : IItemsDataBaseService
    {
        ...
        public async Task<PagedResponse<ItemListItemResponse>> GetItemsAsync(
            int page,
            int pageSize,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            string normalizedItemName = EntityNameNormalizer.NormalizeOptional(itemName) ?? string.Empty;
            string normalizedCategory = NormalizeCategorySlug(category);
            string normalizedPrimaryType = NormalizeOptionalFilter(primaryType);
            string normalizedSecondaryType = NormalizeOptionalFilter(secondaryType);
            string normalizedObjectClass = NormalizeOptionalFilter(objectClass);
            string normalizedVocation = NormalizeOptionalFilter(vocation);
            string normalizedSort = NormalizeItemSort(sort);
            string cacheKey = $"items:list:{normalizedPage}:{normalizedPageSize}:{normalizedItemName}:{normalizedCategory}:{normalizedPrimaryType}:{normalizedSecondaryType}:{normalizedObjectClass}:{normalizedVocation}:{normalizedSort}:{descending}"
                .ToLowerInvariant();
            ...
            return await hybridCache.GetOrCreateAsync(
                ...
                async cancellationToken =>
                {
                    ...
                    {
                        query = query.Where(x =>
                            x.NormalizedName.Contains(normalizedItemName) ||
                            (x.NormalizedActualName != null && x.NormalizedActualName.Contains(normalizedItemName)));
                    }

                    if(!string.IsNullOrWhiteSpace(normalizedCategory))
                    {
                        query = query.Where(x => x.Category != null && x.Category.Slug == normalizedCategory);
                    }

                    if(!string.IsNullOrWhiteSpace(normalizedPrimaryType))
                    {
                        query = query.Where(x => x.PrimaryType != null && x.PrimaryType.ToLower() == normalizedPrimaryType);
                    }

                    if(!string.IsNullOrWhiteSpace(normalizedSecondaryType))
                    {
                        query = query.Where(x => x.SecondaryType != null && x.SecondaryType.ToLower() == normalizedSecondaryType);
                    }

                    if(!string.IsNullOrWhiteSpace(normalizedObjectClass))
                    {
                        query = query.Where(x => x.ObjectClass != null && x.ObjectClass.ToLower() == normalizedObjectClass);
                    }

                    if(!string.IsNullOrWhiteSpace(normalizedVocation))
                    {
                        query = query.Where(x => x.Vocation != null && x.Vocation.ToLower() == normalizedVocation);
                    }

                    int totalCount = await query.CountAsync(cancellationToken);
                    IQueryable<Item> pagedQuery = ApplyItemSorting(query, normalizedSort, descending)
        ...
        private static string NormalizeItemSort(string? sort)
        {
            string normalized = NormalizeOptionalFilter(sort);
        ...
        private static IQueryable<Item> ApplyItemSorting(IQueryable<Item> query, string sort, bool descending)
        {
            return (sort, descending) switch
            {
                ("category", false) => query.OrderBy(x => x.Category != null ? x.Category.Name : null).ThenBy(x => x.Name),
                ("category", true) => query.OrderByDescending(x => x.Category != null ? x.Category.Name : null).ThenBy(x => x.Name),
                ("primary-type", false) => query.OrderBy(x => x.PrimaryType).ThenBy(x => x.Name),
                ("primary-type", true) => query.OrderByDescending(x => x.PrimaryType).ThenBy(x => x.Name),
                ("secondary-type", false) => query.OrderBy(x => x.SecondaryType).ThenBy(x => x.Name),
                ("secondary-type", true) => query.OrderByDescending(x => x.SecondaryType).ThenBy(x => x.Name),
                ("object-class", false) => query.OrderBy(x => x.ObjectClass).ThenBy(x => x.Name),
                ("object-class", true) => query.OrderByDescending(x => x.ObjectClass).ThenBy(x => x.Name),
                ("vocation", false) => query.OrderBy(x => x.Vocation).ThenBy(x => x.Name),
                ("vocation", true) => query.OrderByDescending(x => x.Vocation).ThenBy(x => x.Name),
