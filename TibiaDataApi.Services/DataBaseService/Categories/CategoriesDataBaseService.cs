...
namespace TibiaDataApi.Services.DataBaseService.Categories
{
        ...
        CachingOptions cachingOptions) : ICategoriesDataBaseService
    {
        ...
        public async Task<IReadOnlyList<CategoryListItemResponse>> GetCategoriesByGroupAsync(string group, CancellationToken cancellationToken = default)
        {
            ...
            return await _hybridCache.GetOrCreateAsync(
                $"categories:list:group:{normalizedGroup}",
                async ct => await _db.WikiCategories
                                     .AsNoTracking()
                                     .Where(x => x.IsActive)
                                     .Where(x => x.GroupSlug == normalizedGroup)
                                     .OrderBy(x => x.SortOrder)
                                     .ThenBy(x => x.Name)
                                     .Select(x => new CategoryListItemResponse(
                                         x.Id,
