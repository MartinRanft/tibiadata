        ...
        CachingOptions cachingOptions) : IQuestsDataBaseService
    {
        ...
        private static QuestStructuredDataResponse? CreateStructuredData(WikiArticle article, IReadOnlyDictionary<string, string>? infobox)
        {
            ...
            return new QuestStructuredDataResponse(
                ...
                BuildQuestRequirements(infobox),
                BuildQuestRewards(infobox));
        }
        ...
        private static IReadOnlyList<QuestRequirementResponse> BuildQuestRequirements(IReadOnlyDictionary<string, string>? infobox)
        {
            if(infobox is null || infobox.Count == 0)
            {
                return [];
            ...
            List<QuestRequirementResponse> requirements = [];

            AddRequirement(requirements, "premium", "Premium", GetInfoboxValue(infobox, "premium"));
            AddRequirement(requirements, "level", "Level", GetInfoboxValue(infobox, "level"));
            AddRequirement(requirements, "levelrecommended", "Recommended Level", GetInfoboxValue(infobox, "levelrecommended"));
            AddRequirement(requirements, "levelnote", "Level Note", GetInfoboxValue(infobox, "levelnote"));
            AddRequirement(requirements, "time", "Time", GetInfoboxValue(infobox, "time"));
            AddRequirement(requirements, "timeallocation", "Time Allocation", GetInfoboxValue(infobox, "timeallocation"));
            AddRequirement(requirements, "rookgaardquest", "Rookgaard Quest", GetInfoboxValue(infobox, "rookgaardquest"));

            return requirements;
        }

        private static IReadOnlyList<QuestRewardResponse> BuildQuestRewards(IReadOnlyDictionary<string, string>? infobox)
        {
            string? rawReward = GetInfoboxValue(infobox, "reward");
            if(string.IsNullOrWhiteSpace(rawReward))
            {
                return [];
            }

            string[] rewardParts = rawReward.Split(',', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries);
            List<QuestRewardResponse> rewards = [];

            for(int index = 0; index < rewardParts.Length; index++)
            {
                string value = rewardParts[index].Trim().TrimEnd('.');
                if(string.IsNullOrWhiteSpace(value))
                {
                    continue;
                }

                rewards.Add(new QuestRewardResponse(index + 1, value));
            }

            if(rewards.Count == 0)
            {
                rewards.Add(new QuestRewardResponse(1, rawReward.Trim().TrimEnd('.')));
            }
        ...
        private static void AddRequirement(
            ICollection<QuestRequirementResponse> requirements,
            string key,
            string label,
            string? value)
        {
            ...
            requirements.Add(new QuestRequirementResponse(key, label, value));
        }
